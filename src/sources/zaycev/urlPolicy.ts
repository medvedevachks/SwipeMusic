...
export function zaycevExternalId(href: string): string {
  const url = new URL(href)
  const digits = url.pathname.match(/(\d+)(?:\.html)?$/)
  if (digits?.[1]) {
    return digits[1]
  }

  return Array.from(new TextEncoder().encode(`${url.hostname}${url.pathname}${url.search}`))
